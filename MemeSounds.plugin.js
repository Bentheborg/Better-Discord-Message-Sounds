	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				...
				{re: /fart/gmi, file: "fart.mp3", duration: 470},
				{re: /shit/gmi, file: "fart.mp3", duration: 470},
				{re: /poo/gmi, file: "fart.mp3", duration: 470},
				{re: /ok/gmi, file: "ok.mp3", duration: 470},
				{re: /🗿/gmi, file: "moyai.mp3", duration: 100},
				{re: /moyai/gmi, file: "moyai.mp3", duration: 100},
				{re: /boom/gmi, file: "moyai.mp3", duration: 100}
			];
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/Bentheborg/Better-Discord-Message-Sounds/tree/main/Sounds/"+sound[1].file);
